...
raw_data = pd.read_csv('./data/wine-clustering.csv')
X = raw_data
data = scale(X)
...
def bench_k_means(estimator, name, data):
  ...
  print('%s\t%.2fs\t%i\t%.3f'
    % (name, (time() - t0), estimator.inertia_, silhouette_score))
...
plt.scatter(centroids[:, 0], centroids[:, 1],
  ...
  color='w', zorder=10)
plt.title('K-means clustering on the wine-clustering dataset (PCA-reduced data)\n'
